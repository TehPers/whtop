...
pub(crate) fn get_cpu_frequency(proc_path: &Path, cpu_core_index: usize) -> u64 {
    ...
    s.clear();
    if File::open(proc_path.join("cpuinfo"))
        .and_then(|mut f| f.read_to_string(&mut s))
        .is_err()
    {
        return 0;
...
pub(crate) fn get_physical_core_count(proc_path: &Path) -> Option<usize> {
    let mut s = String::new();
    if let Err(_e) =
        File::open(proc_path.join("cpuinfo")).and_then(|mut f| f.read_to_string(&mut s))
    {
...
pub(crate) fn get_vendor_id_and_brand(proc_path: &Path) -> (String, String) {
    let mut s = String::new();
    if File::open(proc_path.join("cpuinfo"))
        .and_then(|mut f| f.read_to_string(&mut s))
        .is_err()
